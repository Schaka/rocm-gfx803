...
# Marker check, not `apply --check --reverse`: va-reuse-defer-noremap.sh
# touches the same file after this one, and removes the re-map CALL this
# patch adds, so detect on this patch's forward declaration -- the second
# occurrence of the signature line (pristine has only the definition
