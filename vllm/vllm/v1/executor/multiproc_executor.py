    ...
    def monitor_death_pipe(self, death_pipe, shutdown_requested: threading.Event):
        ...
        def death_pipe_monitor(queues_to_shutdown: list[MessageQueue]):
            try:
                ...
                # The parent is gone, so if a peer died as well,
                # destroy_distributed_environment() can block here forever and
                # the process never releases its VRAM. Allow 5s for a clean
                # exit, then force one.
                time.sleep(5.0)
                logger.warning_once(
                    "WorkerProc shutdown hung after parent death; forcing exit."
                )
                os._exit(1)
            except Exception as e:
