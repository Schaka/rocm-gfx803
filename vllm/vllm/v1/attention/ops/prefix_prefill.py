...
def context_attention_fwd(
    ...
    # The 128x64 tile above needs about 80 KB of shared memory for this
    # kernel's Q/K/V/accumulator layout, which overflows the 64 KB per-CU
    # limit on parts like gfx803 -- triton.runtime.errors.OutOfResources at
    # launch. Choose the tile by the device's actual LDS capacity rather than
    # by an arch check, so any 64 KB part reaching this head_dim/block_size
    # combination is covered.
    if is_pow2 and (
        torch.cuda.get_device_properties(q.device).shared_memory_per_block <= 65536
    ):
        BLOCK_M = 32
        BLOCK_N = 32
