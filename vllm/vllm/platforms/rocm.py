...
_ON_GFX803 = "gfx803" in _GCN_ARCH
...
def _capability_from_gcn_arch(gcn_arch: str) -> tuple[int, int] | None:
    ...
    # 8 rather than 9: gfx803 (Polaris) is a supported target here, and it
    # parses to major 8. Upstream's floor assumes no gfx8 part is ever seen.
    if major < 8:
        raise ValueError(
            f"Parsed unknown ROCm architecture from GCN arch '{gcn_arch}': "
            f"major={major}, minor={minor}. "
            f"Major version < 8 is not expected for any supported AMD GPU. "
            f"Please file a vLLM issue with your GPU model."
        )
