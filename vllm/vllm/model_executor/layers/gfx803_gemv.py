...
def gfx803_triton_gemv(
    ...
    NOT CURRENTLY DISPATCHED -- see gfx803_skinny_linear. A synthetic-random-
    data correctness check passes at every real shape in this model
    (including down_proj's K=6144), and a live check during real generation
    also came back clean when forced to run alongside every other decode
    shape too -- but with ONLY down_proj routed through this kernel (its
    intended, narrower use), real end-to-end generation produced degenerate
    output (repeated-token collapse) while the same weight/activation
    pairing checked out fine via LLMM1/rocBLAS. Root cause not isolated:
    every attempt to capture real per-call activations for direct
    comparison (CUDA-graph capture bypasses this kernel's own Python
    dispatch on replay; forcing eager execution to avoid that triggers an
...
def gfx803_skinny_linear(
    ...
    rocblas_hssgemv_strided_batched otherwise. Returns fp32 (matches both
    kernels' accumulate type -- callers on the fp16-through path should cast
    back). See module docstring for why K gates LLMM1's use, and
    gfx803_triton_gemv's own docstring for why it's defined but not
    dispatched here.
    """
    M, K = weight.shape
    if K <= _LLMM1_MAX_K and M % 2 == 0:
        ...
        if bias is not None:
            ...
            out = out + bias
        return out
    return gfx803_skinny_gemv(x, weight, bias).to(x.dtype)
