...
"$VENV/bin/python3" -c "import importlib.metadata as m; [print(f'{p}=={m.version(p)}') for p in ('torch', 'torchvision', 'torchaudio', 'onnxruntime-migraphx')]" > /opt/pip-constraints.txt
