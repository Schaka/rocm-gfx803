...
```bash
# rocm7.14 (this line) -- versioned tag
docker pull ghcr.io/schaka/rocm-migraphx-ort-torch-builder:rocm7.14-gfx803

# rocm7.14 -- newest successful build of this line (suffixed; the
# unsuffixed :latest-gfx803 tag now belongs to the rocm10 main line)
docker pull ghcr.io/schaka/rocm-migraphx-ort-torch-builder:latest-gfx803-rocm7.14
...
This is the **rocm7.14 line's** README, archived under `rocm7.14/`. The
repo's main line is now ROCm 10.0 at the root (see the top-level
`README.md`); the layout below describes where this line sat in the repo
before that move.

```
rocm-gfx803/
├── rocm7.14/
│   ├── Dockerfile              # ROCm 7.14 (TheRock), this line
│   ├── .dockerignore
...
**rocm7.14 was the actively developed line until the 10.0 move, and is now
the immediately-preceding, hardware-verified one.** ROCm 7.14 is a TheRock
meta-release, not a classic per-repo tag -- every component is pinned to a
release *branch*, not a frozen commit: ROCR-Runtime, rocBLAS and MIOpen (now
the `rocm-systems`/`rocm-libraries` monorepos) track `release/therock-7.14`,
MIGraphX tracks `release/rocm-rel-7.14`, same `release/rocm-rel-<major.minor>`
convention this repo's mainline (`rocm-migraphx-ort-builder`) uses for its
own manual releases -- no nightly scheduling, no prebuilt wheels, just "build
from the named release branch's current tip when someone runs it." See
`MIGRATION_NOTES.md` for how those refs were resolved and why.
...
where new investigation happens. The lines are deliberately independent
copies, not a shared asset; a shared file would mean a bug found while
chasing 7.14 could reach the hardware-verified 6.4.4 build. Once 7.14 (and
the 10.0 line that supersedes it) are confirmed at least as solid as 6.4.4
across the board, the lines get diffed and consciously merged back together
-- see "Convergence" below.

## Status

- **rocm7.14 (this line)**: builds end-to-end and is hardware-verified.
...
Once this line (and the 10.0 line that supersedes it) are confirmed at
least as solid as `rocm6.4.4/` -- correctness-suite clean (already true),
ORT suite parity (one open item, see Status above), and comparable real-model
results (already true) -- the lines get diffed and deliberately merged:
whatever's still 6.4.4/7.14-only that should generalize moves up, and the
independent copies collapse back into a shared structure. Not done yet; all
lines are still receiving patches independently.
