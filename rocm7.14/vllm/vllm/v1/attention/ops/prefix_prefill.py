...
def context_attention_fwd(
    ...
    # gfx803 (Polaris, 64KB LDS/CU): the 128x64 tile above needs 80KB of
    # shared memory for this kernel'''s Q/K/V/accumulator layout, which
    # overflows the 64KB hard limit (triton.runtime.errors.OutOfResources,
    # confirmed on real hardware). Drop to the same 32x32 tile the
    # non-power-of-2 branch above already uses whenever the device'''s
    # actual LDS capacity can'''t fit the bigger tile, instead of
    # hardcoding a gfx803 check -- this covers any other 64KB-LDS part
    # exercising this same head_dim/block_size combination.
    if is_pow2 and torch.cuda.get_device_properties(q.device).shared_memory_per_block <= 65536:
        BLOCK_M = 32
        BLOCK_N = 32
