...
The split kernel itself (`kernel_paged_attention_2d_split`) is now
Triton-only as a fallback -- the live path is a hand-written HIP kernel
(`gfx803_attn_split.hip`/`libgfx803attn.so`, ctypes-bridged, same pattern
as `gfx803_prefill_gemm.py`). Reason: pulling this Triton kernel's
compiled ISA straight from `~/.triton/cache` found it pinned at 256/256
VGPR with 637 spilled registers at this model's HEAD_SIZE_PADDED=256 --
Triton materializes a full-width K/V tile and accumulator per iteration
with no way to chunk the head dimension (no list comprehensions, no
tensor slicing, no loop-carried tensor collections in Triton's frontend --
all three tried and rejected). The HIP kernel processes one KV token at a
time with a wavefront-cooperative `__shfl_xor` dot-product reduction
instead of a tile load, so per-thread state never exceeds a handful of
floats: 61 VGPR, 0 spills, verified bit-for-bit (fp32-rounding-level,
<3e-5 max abs diff) against this Triton kernel across seq_len 1-4096 and
NUM_KV_SPLITS 1-32, and 1.88x faster in isolated microbenchmarking at
this model's real 2229-token decode context. Toggle back to the Triton
path with `VLLM_GFX803_ATTN_HIP_KERNEL=0` if the HIP path ever needs to
be ruled out on a real workload.
"""
...
_ATTN_LIB_PATH = __file__.rsplit("/", 1)[0] + "/libgfx803attn.so"
_USE_HIP_ATTN_KERNEL = os.environ.get("VLLM_GFX803_ATTN_HIP_KERNEL", "1") != "0"


@functools.lru_cache(maxsize=1)
def _attn_lib() -> ctypes.CDLL:
    lib = ctypes.CDLL(_ATTN_LIB_PATH, mode=ctypes.RTLD_GLOBAL)
    lib.gfx803_attn_split_launch.argtypes = [
        ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,  # partial_out/m/l
        ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,  # query/key_cache/value_cache
        ctypes.c_void_p, ctypes.c_void_p,  # block_tables, seq_lens
        ctypes.c_float,  # scale
        ctypes.c_int, ctypes.c_int, ctypes.c_int,  # num_seqs, num_kv_heads, num_query_heads
        ctypes.c_int, ctypes.c_int,  # num_queries_per_kv, num_queries_per_kv_padded
        ctypes.c_int,  # block_table_stride
        ctypes.c_longlong, ctypes.c_longlong,  # query_stride_0/1
        ctypes.c_int,  # physical_block_size
        ctypes.c_int, ctypes.c_int, ctypes.c_int,  # head_size, head_size_padded, x
        ...
        ctypes.c_int, ctypes.c_int,  # num_kv_splits, head_slots
        ctypes.c_void_p,  # stream
    ]
    lib.gfx803_attn_split_launch.restype = None
    return lib
...
def gfx803_split_decode_attention(
    ...
    if _USE_HIP_ATTN_KERNEL:
        _attn_lib().gfx803_attn_split_launch(
            partial_out.data_ptr(), partial_m.data_ptr(), partial_l.data_ptr(),
            query.data_ptr(), key_cache.data_ptr(), value_cache.data_ptr(),
            block_table.data_ptr(), seq_lens.data_ptr(),
            scale,
            num_seqs, num_kv_heads, num_query_heads,
            num_queries_per_kv, num_queries_per_kv_padded,
